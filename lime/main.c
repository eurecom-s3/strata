...
extern void noinline lime_pre_write(void *v);
extern void noinline lime_post_write(void *v);
extern int lime_skip_range;
extern u64 lime_page_timestamp;
...
{
    lime_page_timestamp = ktime_get_raw_fast_ns();
    lime_skip_range = 0;
...
// Patched to always write a integer number of PAGEs
static ssize_t write_padding(size_t s) {
    int i;
    ssize_t r;
    int num_pages = s/PAGE_SIZE;
    if(s % PAGE_SIZE) num_pages++;

    memset(vpage, 0, PAGE_SIZE);
    for(i=0;i<num_pages;i++){
        r = write_vaddr(vpage, PAGE_SIZE);

        if(r!=PAGE_SIZE) {
            DBG("Error writing zero page: %zd", r);
            return r;
        }
    }
...
void write_range(struct resource * res) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,18)
...
EXPORT_SYMBOL(lime_pre_write);
EXPORT_SYMBOL(lime_post_write);
